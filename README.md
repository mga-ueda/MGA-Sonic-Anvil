...
**複数ファイルはタブのほか、タイル表示で聴き比べられます。** Ctrl+T で左右・上下・上下左右を巡回、右クリックで直接指定。設定で、開いたあとに自動で並べる既定配置も選べます。タイル表示中は Ctrl+F でタブ名の検索（空白 AND・`|` OR）ができ、ヒットしないタイルは暗くなってすりガラスで覆われます。Alt+Enter でヒット以外を閉じます（未保存は残す。確認なし。フィルターは解除）。

**Wwise へは Wave 単体モードで渡せます。** レベルメーター上の WAAPI をオンにし、作成先を選んで EXPORT すると、元波形を Originals へ書き、Music Playlist Container（-A / -L / -E とサンプルループ。Custom Cue にはしない）として取り込みます。Play -E はループ折り返しのプレビューと、EXPORT 時の Play post-exit に使います。

**MP3 にも書けます。** 録音そのものは PCM のままです。設定の LAME パスが有効ならユーザー用意の `lame.exe`、空欄または無効なら Windows（既定 192 kbps）。多チャンネルは今のスピーカー配置で聴いている 2ch 畳みを書きます。タブの右クリックで、今の編集内容を Wave / MP3 として書き出せます（複数タブはフォルダ指定。未保存のままで可）。
