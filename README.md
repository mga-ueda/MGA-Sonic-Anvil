...
**複数ファイルはタブのほか、タイル表示で聴き比べられます。** Ctrl+T で左右・上下・上下左右を巡回、右クリックで直接指定。設定で、開いたあとに自動で並べる既定配置も選べます。タイル表示中は Ctrl+F でタブ名の検索（空白 AND・`|` OR）ができ、ヒットしないタイルは暗くなってすりガラスで覆われます。
