...
internal static partial class UiStrings
{
    ...
    public static string LabelLibraryListColumnsHint => Get(
        "チェックした列だけを表示します。見出しをドラッグした並びも覚えます。ファイル名は外せません。",
        "Only checked columns are shown. Dragging headers to reorder is remembered. The file name column cannot be turned off.");
    ...
    public static string ButtonSaveAllAndClose => Get("すべて保存して閉じる", "Save all and close");
    public static string ButtonDiscardAllAndClose => Get("すべて保存せずに閉じる", "Close without saving any");
    ...
    public static string SaveFolderTitle => Get("保存先フォルダ", "Save folder");
    ...
    public static string ConfirmSaveBatchHint(int remaining) => Format(
        "未保存のタブが {0} 件あります。まとめて閉じることもできます。",
        "{0} tab(s) have unsaved changes. You can close them all at once.",
        remaining);
    ...
    public static string OverlaySave => Get("保存しています", "Saving");
    ...
    public static string TooltipSave => Get("保存 (Ctrl+S)  複数選択時は選択ファイル全部", "Save (Ctrl+S)  all selected files when several tabs are selected");
    ...
    public static string TooltipRangeClick => Get("範囲クリック (K)", "Range click (K)");
    public static string TooltipOpen => Get("開く (Ctrl+O)", "Open (Ctrl+O)");
    public static string TooltipNew => Get("新規 (Ctrl+N)", "New (Ctrl+N)");
    public static string TooltipSaveAs => Get("名前を付けて保存 (Ctrl+Shift+S)  複数選択時は保存先フォルダ", "Save As (Ctrl+Shift+S)  pick a folder when several tabs are selected");
    ...
    public static string TipRangeClick => Get(
        "範囲クリック (K)\n等分マーカー／リージョン位置で再生クリック。ボタンまたは K でオン／オフ（既定オフ・自動ではオンにならない）",
        "Range click (K)\nPlayback clicks at equal-divide markers/regions. Toggle with the button or K (off by default; never auto-on)");
    public static string TipSaveAs => Get(
        "名前を付けて保存 (Ctrl+Shift+S)\n単体は保存ダイアログ。複数選択時は保存先フォルダを選び、各ファイルを Wave で保存してタブのパスを付け替える（書き出しとは違い、保存後のファイルが今のタブになる）",
        "Save As (Ctrl+Shift+S)\nOne file opens a Save dialog. Several selected tabs ask for a folder, write each as Wave, and point the tabs at those files (unlike Export, the tab follows the new file)");
    ...
    public static string TipWaveform => Get(
        ...
        + "F10 はプレイヤー（リスト＋短い波形。操作の詳細はプレイリスト／ツリーの Tips。F9（プレイリストと波形だけ。デスクトップの占有面積を節約するとき）も同様）。プレイヤー中の波形はシークと選択だけ（右クリックもメニューキー／Shift+F10 も出さない。マーカー／リージョン／ループは出さない。範囲選択は反転＋半透明。拡大しない。表示用のピークだけ作る。短いファイルは粗くしない。スペクトラム／ラウドネス曲線ビューには切り替えない）。編集コマンドは使えない。他モードのファイルはリストへ引き継ぐ。抜けるときは再生を止めてから、選択したファイルだけ残す。プレイヤーのまま終了したときは、読み込んだファイルは覚えない。ウィンドウの位置とサイズは通常モードと別々に覚え、切り替えで戻す。引数に mp3／m4a が1つでも混ざるとプレイヤーで起動し、それらのファイルをリストへ載せる。もう一度 F10 で戻す。",
        ...
        + "F10 is the player (list + short waveform; see the playlist / tree Tips for details, including F9, which shows only the playlist and waveform to save desktop space). In player mode the waveform only seeks and selects (no right-click, menu key, or Shift+F10 menu; no marker / region / loop overlay; inverted translucent selection; no zoom; display-resolution peaks only; short files stay dense; spectrogram / loudness-curve views stay off). Edit commands are blocked. Files from other modes are listed. Leaving stops playback first, then keeps only the selected files. Quitting while still in player mode does not remember the loaded files. Window position and size are remembered separately from editor mode and restored when you switch. If any launch argument is an MP3 or M4A, the app starts in player mode with those files listed. F10 again restores.");
    ...
    public static string LibraryColumnDate => Get("日付", "Date");
    ...
    public static string LibraryColumnLabel(LibraryFileColumn column) => column switch
    {
        ...
        LibraryFileColumn.Date => LibraryColumnDate,
    ...
    public static string TipLibraryExplorerSearch => Get(
        "ファイル名とフォルダ名で絞り込みます。ヒットしたファイル／フォルダと、ルートまでの祖先だけ出します。空白区切りで AND、| で OR。Ctrl+F でここへ。Enter で決定してツリーへ戻ります。Esc は入力を破棄します。ツリーで Enter すると見えているフォルダだけをプレイリストへ載せます。ファイル名がヒットしたフォルダはそのファイルだけ、フォルダ名がヒットしたフォルダは中身すべてです。",
        "Filter by file name and folder name. Only hits and their ancestors up to the root stay in the tree. Space-separated terms = AND, | = OR. Ctrl+F focuses here. Enter applies and returns to the tree. Esc discards the typed text. Enter on the tree adds only visible folders: matching files when the hit is a file name, or everything in a folder whose name matched.");
    public static string TipLibraryPlaylistSearch => Get(
        "プレイリストのすべての列が対象です。ヒットした行だけ残します。空白区切りで AND、| で OR。Ctrl+F でここへ。Enter で決定してリストへ戻ります。Esc は入力を破棄します。",
        "Searches every playlist column and keeps matching rows. Space-separated terms = AND, | = OR. Ctrl+F focuses here. Enter applies and returns to the list. Esc discards the typed text.");
    ...
    public static string TipLibraryList => Get(
        "プレイヤーのファイル一覧。左のツリーでフォルダを選んでもリストへは自動で載せない。フォルダをリストへドロップすると配下ごと追加する。Delete または右クリック／メニューキー「プレイリストからクリア」で選択行をリストから外す（ファイルは消さない。Ctrl+Z で戻せる。プレイヤーを抜けると忘れる。Ctrl+W では閉じない）。外へドラッグ、または Ctrl+C／右クリックのコピーで、選択ファイルをコピーできる。右クリックまたはメニューキーでエクスプローラーで開く。ソート中の列は見出しに小さい ▼▲ を出し、有効な方向だけシアン。既定はアルバムでグループし、グループごとにジャケットを出す。グループでタイトル／アーティスト／アルバムなどでもまとめる。入ると1曲めを選んで再生する（引数起動も同じ）。プレイリストが空のときはレベルメーター・スペアナ・ラウドネス・ゴニオ／サラウンドメーターを出さない（入ったときは最初から消す）。空のときはライブラリ（フォルダツリー）にフォーカス。R でランダム再生（一巡まで同じ曲を繰り返さない。空でも切替可。既定オフ。プレイヤーを抜けるとオフ。設定には残さない）。停止中の選択は波形だけ出し、再生はしない。曲を追加したときは1秒でフェードインし、音声信号で動き始めたら即表示する。リストが空になったら1秒でフェードアウトする。再生開始と同時に次の曲を先読みし、終わったら隙間なく次を再生する。最後の次は先頭に戻って再生を続ける。↑↓ でファイル選択。Home／End でリストの先頭／末尾。PageUp／PageDown でページ送り（再生中はキーを離すとその曲へ。停止中は選択だけ。Shift で範囲、Ctrl+クリックで追加、Ctrl+A で全選択）。F1 または Ctrl+←／Ctrl+↑ でツリー、F2 または Ctrl+↓ でお気に入り、F3 または Ctrl+→ でこのリストにフォーカス。Tab でツリー→お気に入り→プレイリスト（Shift+Tab は逆。フォーカス中は上端に明るいグラデーション）。F9 でプレイリストと波形だけを出す（デスクトップの占有面積を節約したいとき。どのモードからでも。ライブラリ／お気に入り／メーター／アナライザ／トランスポート／ステータスバーは出さない。Alt+S（Silent Skip）は効く。もう一度 F9 で通常のエディタへ戻す（再生を止めてから、選択したファイルだけ残す）。F10 で通常のプレイヤー＝F10 表示に戻す（再生は続ける））。再生中にクリックした行はその曲へ切り替える。ダブルクリック、Enter、またはトランスポートの再生で先頭から再生し、終わったら次の曲へ進む。Space はその曲だけ先頭から再生し、終わったら停止する（再生中は開始位置へ戻して止める）。テンキーはどのペインからでも（NumLock オン）0＝再生／一時停止、1＝早戻し、3＝早送り（推移中は-4dB）、4＝前の曲、5＝再生し直し、6＝次の曲、7／9＝5秒戻し／送り（0.75秒クロスフェード。押しっぱなしは少し待ってから繰り返す）。左右キーの早送り／巻き戻しはしない。キーボード上段の 0–9 は割合ジャンプのまま。編集はできない。タグと時間は登録時に読む。MP3／M4A／WAVE／AIFF はフル PCM 展開せずストリーム再生（M4A はプレイヤー専用）。ジャケットは選択時に読む。埋め込みが無ければ No Image のプレースホルダ。本物のジャケットがあるときはウィンドウ全体の背面を大きくぼかして90度ずつ揺らぐ。WAVE 等は差し替え不可。WAAPI は無効（入る前にオンなら、抜けたときに戻す）。プレイヤーを抜けると再生を止めてから、選択したファイルだけ残る。プレイヤーのまま終了したときは、読み込んだファイルは覚えない。",
        "Player file list. Selecting a folder in the tree on the left does not add it to the list. Drop a folder onto the list to add recursively. Delete, right-click, or the menu key Clear from Playlist removes selected rows from the list (files are never deleted. Ctrl+Z undoes it; leaving the player forgets undo. Ctrl+W does not close). Drag outside the app, or Ctrl+C / right-click Copy, copies the selected files. Right-click or the menu key can open the location in Explorer. The sorted column shows small ▼▲ in the header, and only the active direction is cyan. Groups default to album, with a jacket beside each group. Group by title, artist, album, and so on. Entering selects and plays the first track (the same on launch with arguments). While the playlist is empty, the level meter, spectrum, loudness, gonio, and surround meter stay hidden (they start hidden when you enter player). An empty playlist focuses the library (folder tree). R toggles shuffle (no repeats until the playlist has played once; works on an empty list; off by default; leaving player turns it off; not saved). While stopped, selecting a track shows its waveform without playing. Adding tracks fades them in over 1 second; when they start moving with the audio they appear immediately. Clearing the list fades them out over 1 second. The next track is read ahead as soon as playback starts, then plays without a gap. After the last, playback returns to the first and keeps going. ↑↓ select files. Home / End jump to the first / last track. Page Up / Page Down page the list (while playing, releasing the key switches to that track; while stopped, selection only. Shift for a range, Ctrl+click to add, Ctrl+A for all). F1 or Ctrl+← / Ctrl+↑ focuses the tree, F2 or Ctrl+↓ Favorites, F3 or Ctrl+→ this list. Tab cycles tree → Favorites → playlist (Shift+Tab reverse; the active pane shows a brighter fade at the top). F9 shows only the playlist and waveform (to save desktop space; from any mode; library, Favorites, meters, analyzers, transport, and status bar stay hidden; Alt+S Silent Skip still works; F9 again returns to the editor (stops playback, then keeps only the selected files); F10 returns to the normal F10 player and keeps playback going). While playing, a click switches to that track. Double-click, Enter, or the transport Play button restarts from the beginning and continues to the next track when it ends. Space plays only that track from the beginning and stops when it ends (while playing, stops and returns to the start). The numpad (NumLock on) works from any pane: 0 play / pause, 1 rewind, 3 fast-forward (−4 dB while held), 4 previous track, 5 restart, 6 next track, 7 / 9 skip 5 s back / forward with a 0.75 s crossfade (hold repeats after a short wait). Arrow keys do not shuttle. Top-row 0–9 still jump by view percent. Editing is blocked. Tags and duration are read when files are listed. MP3 / M4A / WAVE / AIFF stream without a full PCM decode (M4A is player-only). Jacket images load on select; a No Image placeholder is used when none is embedded. A real jacket is also stretched and blurred as a slowly turning background across the whole player window. WAVE and similar cannot replace art. WAAPI is off (if it was on, leaving player turns it back on). Leaving player mode stops playback first, then keeps only the selected files. Quitting while still in player mode does not remember the loaded files.");
    public static string TipLibraryJacket => Get(
        "本物のジャケットがあるときは、ウィンドウ全体の背面に縦横比を崩して置き、大きくぼかして90度ずつ4向きをゆっくりクロスフェードしながら揺らぐ（リスト・波形・トランスポート・メーター・ステータスバーまで一枚）。ジャケットが変わったときは1秒でクロスフェードする。同じ画像のまま曲を変えても、フェードも向きの切り替えもやり直さない。埋め込みが無ければジャケット枠は No Image。背面は既定の背景色。",
        "When a real jacket is set, it is stretched across the whole player window (aspect may change) and used as a heavily blurred background that slowly crossfades through four 90-degree turns while drifting — list, waveform, transport, meters, and the status bar as one wash. A new jacket crossfades in 1 second. Skipping tracks that keep the same jacket does not restart the fade or the turn cycle. The jacket frame shows No Image when none is embedded. The wash falls back to the default color.");
    public static string TipLibraryColumns => Get(
        "設定の表示項目で、リストに出す列を選ぶ。チェックした列だけ表示。見出しをドラッグした並びも次の起動で覚える。ファイル名の列は外せない。",
        "Choose list columns in Settings → Shown. Only checked columns appear. Dragging headers to reorder is remembered. The file name column stays on.");
    public static string TipLibraryExplorer => Get(
        "フォルダツリー。ルートは設定のプレイヤーで複数登録できる（既定はマイミュージック。上から順。登録・削除・並び替え）。選んだ場所・展開状況と、お気に入りとの仕切り位置は覚える。区切りとスクロールバーは近づけると出し、マウスが約3秒止まると消す。Ctrl+クリック／Shift+クリック／Shift+↑↓で複数フォルダを選択。右クリックまたはメニューキーで、プレイリストをクリアして追加、プレイリストへ追加、お気に入りへ追加、コピー、エクスプローラーで開く。フォルダを選んでもリストへは自動で載せない。フォルダをリストへドロップすると配下ごと追加する。ダブルクリックは配下を再帰的に追加する。Enter はプレイリストをクリアしてから、そのフォルダ配下を再帰的に載せる（検索中は見えているフォルダだけ。ファイル名ヒットはそのファイルだけ、フォルダ名ヒットはそのフォルダ内すべて。深さ優先で最初に登録した曲から再生。再生が始まるとプレイリストをアクティブにする）。Shift+Enter はクリアせず追加する（フォルダを見つけ次第、1曲ずつ。裏で集める）。文字キーは表示名の先頭一致でフォルダへジャンプする（続けて打つと絞り込む。約1秒空くと打ち直し。R はランダム再生の切替。上段の数字は割合ジャンプのまま。プレイリストには載せない。Ctrl+F の検索とは別）。* は選択フォルダ以下をすべて展開、/ は選択フォルダ以下を閉じる（エクスプローラーと同じ）。F1 または Ctrl+←／Ctrl+↑ でこのツリーにフォーカス。Tab でお気に入りへ（Shift+Tab はプレイリスト。フォーカス中は上端に明るいグラデーション）。←→ はフォルダの展開と折りたたみ（早送り／巻き戻しにはならない）。テンキーの再生操作はツリーにフォーカスがあっても効く。外へドラッグ、または Ctrl+C／右クリックのコピーで、選択フォルダをコピーできる。複製・削除・リネームはできない。",
        "Folder tree. Roots can be registered in Settings → Player (default Music, top to bottom; add, remove, reorder). The last place, which folders were expanded, and the split with Favorites, are remembered. Splitters and scrollbars appear when the pointer nears them, then fade out after about 3 s of no movement. Ctrl+click, Shift+click, or Shift+↑↓ selects multiple folders. Right-click or the menu key can clear the playlist and add, add to the playlist, add to Favorites, copy, or open in Explorer. Selecting a folder does not add it to the list. Drop a folder onto the list to add recursively. Double-click appends files from that folder and its subfolders. Enter clears the playlist, then adds that folder recursively (while searching, only visible folders: matching files when the hit is a file name, or everything in a folder whose name matched; depth-first; playback starts with the first file registered and focuses the playlist). Shift+Enter appends only (as each folder is found, tracks are added one by one in the background). Letter keys jump to a folder whose display name starts with what you type (keep typing to narrow; about 1 s idle starts over; R toggles shuffle; top-row digits stay view-percent jump; does not add to the playlist; separate from Ctrl+F search). * expands all under the selection, / collapses it (same as Explorer). F1 or Ctrl+← / Ctrl+↑ focuses this tree. Tab moves to Favorites (Shift+Tab to the playlist; the active pane shows a brighter fade at the top). ←→ expands or collapses folders (not shuttle). Numpad playback keys still work while the tree has focus. Drag outside the app, or Ctrl+C / right-click Copy, copies the selected folders. Duplicate, delete, and rename are not available.");
    public static string TipLibraryFavorites => Get(
        "お気に入り。名前だけ表示（フルパスは出さない）。複数選択可（Shift+クリック、Shift+↑↓、Ctrl+クリック）。プレイリストへドラッグで追加。外へドラッグ、または Ctrl+C／右クリックのコピーで、実ファイルをコピーできる。Enter でプレイリストをクリアして追加し即再生（見つけ次第、1曲ずつ。ツリーと同じ。再生が始まるとプレイリストをアクティブにする）。Shift+Enter とダブルクリックは追加のみ（再生しない。見つけ次第、1曲ずつ）。右クリックまたはメニューキーで、プレイリストをクリアして追加、プレイリストへ追加、お気に入りから削除、コピー、エクスプローラーで開く（登録を外してもファイルは消さない。Delete でも外せる）。F2 または Ctrl+↓ でここへフォーカス。Tab でプレイリストへ（Shift+Tab はツリー。フォーカス中は上端に明るいグラデーション）。テンキーの再生操作はここでも効く。",
        "Favorites. Names only (no full path). Multi-select (Shift+click, Shift+↑↓, Ctrl+click). Drag onto the playlist to add. Drag outside the app, or Ctrl+C / right-click Copy, copies the real files. Enter clears the playlist, adds, and plays (one track at a time as found; same as the tree; focuses the playlist when playback starts). Shift+Enter and double-click append only (do not play; one track at a time as found). Right-click or the menu key can clear the playlist and add, add to the playlist, remove from Favorites, copy, or open in Explorer (removing a favorite never deletes the file; Delete also unregisters it). F2 or Ctrl+↓ focuses here. Tab moves to the playlist (Shift+Tab to the tree; the active pane shows a brighter fade at the top). Numpad playback keys still work here.");
    ...
    public static string FormatFileDate(DateTime? value)
    {
        if (value is not { } date || date == DateTime.MinValue)
        {
            return string.Empty;
        }

        return date.ToString("yyyy/MM/dd HH:mm", System.Globalization.CultureInfo.InvariantCulture);
    }
