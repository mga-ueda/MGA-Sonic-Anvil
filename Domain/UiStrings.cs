...
internal static partial class UiStrings
{
    ...
    public static string TipWaveform => Get(
        ...
        + "F10 はプレイヤー（リスト＋短い波形。操作の詳細はプレイリスト／ツリーの Tips。F9 ミニマムも同様）。プレイヤー中の波形はシークと選択だけ（右クリックもメニューキー／Shift+F10 も出さない。マーカー／リージョン／ループは出さない。範囲選択は反転＋半透明。拡大しない。表示用のピークだけ作る。短いファイルは粗くしない。スペクトラム／ラウドネス曲線ビューには切り替えない）。編集コマンドは使えない。他モードのファイルはリストへ引き継ぐ。抜けるときは再生を止めてから、選択したファイルだけ残す。プレイヤーのまま終了したときは、読み込んだファイルは覚えない。ウィンドウの位置とサイズは通常モードと別々に覚え、切り替えで戻す。引数に mp3／m4a が1つでも混ざるとプレイヤーで起動し、それらのファイルをリストへ載せる。もう一度 F10 で戻す。",
