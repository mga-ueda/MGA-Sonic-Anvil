...
internal static partial class UiStrings
{
    ...
    public static string TipWaveform => Get(
        ...
        + "F10 はプレイヤーモード。全画面にはしない。タブは出さず、上段は左にフォルダツリー、中央がファイルリスト。下段は固定の低い波形。全体波形は出さない。既定はアルバムでグループし、グループごとにジャケットを出す。再生や曲送りではリストを作り直さない。入ると1曲めを選んで再生する（引数起動も同じ）。プレイリストが空のときはレベルメーター・スペアナ・ラウドネス・ゴニオ／サラウンドメーターを出さない（入ったときは最初から消す）。曲を追加したときは1秒でフェードインし、音声信号で動き始めたら即表示する。リストが空になったら1秒でフェードアウトする。再生開始と同時に次の曲を先読みし、終わったら隙間なく次を再生する。最後の次は先頭に戻って再生を続ける。↑↓ でファイル選択。Home／End でリストの先頭／末尾。PageUp／PageDown でページ送り（再生中はキーを離すとその曲へ。停止中は選択だけ。複数選択可）。F1 でツリー、F2 でお気に入り、F3 でプレイリストにフォーカス。Tab でツリー→お気に入り→プレイリスト（Shift+Tab は逆。フォーカス中は上端に明るいグラデーション）。ツリーでは ←→ でフォルダを展開／折りたたみ（プレイヤーでは左右キーの早送り／巻き戻しはしない）。* は選択フォルダ以下を全展開、/ で閉じる。Enter で再生／先頭から再生し直し。テンキーはどのペインからでも（NumLock オン）0＝再生／一時停止、1＝早戻し、3＝早送り（推移中は-4dB）、4＝前の曲、5＝再生し直し、6＝次の曲、7／9＝5秒戻し／送り（0.75秒クロスフェード。押しっぱなしは少し待ってから繰り返す）。キーボード上段の 0–9 は表示範囲の割合ジャンプのまま。波形はシークと選択だけ（右クリックもメニューキー／Shift+F10 も出さない。マーカー／リージョン／ループは出さない。範囲選択は反転＋半透明。拡大しない。表示用のピークだけ作る。短いファイルは粗くしない。スペクトラム／ラウドネス曲線ビューには切り替えない）。編集コマンドは使えない。他モードのファイルはリストへ引き継ぐ。抜けるときは再生を止めてから、選択したファイルだけ残す。プレイヤーのまま終了したときは、読み込んだファイルは覚えない。ウィンドウの位置とサイズは通常モードと別々に覚え、切り替えで戻す。波形は全件走査しないが、タグと時間は登録時に読む。列の出し分けと見出しのドラッグ順は設定の表示項目（状態は覚える）。左のツリーは既定でマイミュージック、選んだ場所・展開状況と、お気に入りとの仕切り位置は覚える。区切りとスクロールバーは近づけると出し、マウスが約3秒止まると消す。フォルダを選んでもリストへは自動で載せない。ツリーの Enter はプレイリストをクリアしてそのフォルダ配下を載せる（検索中は見えているフォルダだけ。ファイル名ヒットはそのファイルだけ、フォルダ名ヒットはそのフォルダ内すべて。深さ優先で最初に登録した曲から再生）。Shift+Enter とダブルクリック、リストへのフォルダドロップは配下ごと追加する。ツリー・お気に入り・プレイリストから外へドラッグまたは Ctrl+C でコピーできる。右クリックまたはメニューキーでコピーとエクスプローラーで開く。MP3／M4A／WAVE／AIFF はフル PCM 展開せずストリーム再生する（M4A はプレイヤー専用）。ジャケットは選択時に読む。埋め込みが無ければ No Image のプレースホルダ。本物のジャケットがあるときはウィンドウ全体の背景をぼかして揺らぐ。WAAPI は無効（入る前にオンなら、抜けたときに戻す）。引数に mp3／m4a が1つでも混ざるとプレイヤーで起動し、それらのファイルをリストへ載せる。もう一度 F10 で戻す。",
        ...
        + "F10 is a player, not fullscreen: no tabs, file list on top (folder tree left, list center) and a short fixed waveform below. The overview is hidden. Groups default to album, with a jacket beside each group. Playback and skip do not rebuild the list. Entering selects and plays the first track (the same on launch with arguments). While the playlist is empty, the level meter, spectrum, loudness, gonio, and surround meter stay hidden (they start hidden when you enter player). Adding tracks fades them in over 1 second; when they start moving with the audio they appear immediately. Clearing the list fades them out over 1 second. The next track is read ahead as soon as playback starts, then plays without a gap. After the last, playback returns to the first and keeps going. ↑↓ select files. Home / End jump to the first / last track. Page Up / Page Down page the list (while playing, releasing the key switches to that track; while stopped, selection only; multi-select allowed). F1 focuses the tree, F2 Favorites, F3 the playlist. Tab cycles tree → Favorites → playlist (Shift+Tab reverse; the active pane shows a brighter fade at the top). On the tree, ←→ expands or collapses folders (player mode does not shuttle with the arrow keys). * expands all under the selection, / collapses it. Enter plays / restarts from the beginning. The numpad (NumLock on) works from any pane: 0 play / pause, 1 rewind, 3 fast-forward (−4 dB while held), 4 previous track, 5 restart, 6 next track, 7 / 9 skip 5 s back / forward with a 0.75 s crossfade (hold repeats after a short wait). Top-row 0–9 still jump by view percent. The waveform only seeks and selects (no right-click, menu key, or Shift+F10 menu; no marker / region / loop overlay; inverted translucent selection; no zoom; display-resolution peaks only; short files stay dense; spectrogram / loudness-curve views stay off). Edit commands are blocked. Files from other modes are listed. Leaving stops playback first, then keeps only the selected files. Quitting while still in player mode does not remember the loaded files. Window position and size are remembered separately from editor mode and restored when you switch. PCM is not scanned for every file; tags and duration are read on register. Playlist columns and header order are chosen in Settings → Shown (remembered). The tree on the left starts at Music and remembers the last folder, which folders were expanded, and the split with Favorites. Splitters and scrollbars appear when the pointer nears them, then fade out after about 3 s of no movement; selecting a folder does not add it to the list. Enter on the tree clears the playlist, then adds that folder recursively (while searching, only visible folders: matching files when the hit is a file name, or everything in a folder whose name matched; depth-first; playback starts with the first file registered). Shift+Enter, double-click, or dropping a folder onto the list, adds them recursively. Drag from the tree, Favorites, or playlist to copy outside the app, or press Ctrl+C. Right-click or the menu key can copy or open the location in Explorer. MP3 / M4A / WAVE / AIFF stream without a full PCM decode (M4A is player-only). Jacket images load on select; a No Image placeholder is used when none is embedded. A real jacket is stretched and blurred across the whole player window. WAAPI is off (if it was on, leaving player turns it back on). If any launch argument is an MP3 or M4A, the app starts in player mode with those files listed. F10 again restores.");
