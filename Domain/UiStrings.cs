...
internal static partial class UiStrings
{
    ...
    public static string TipLibraryShuffle => Get(
        "ランダム再生 (R)\nプレイリストを一通り再生し終わるまで同じ曲を繰り返さない。すべて再生したら再度抽選する。既定はオフ。",
        "Shuffle (R)\nDoes not repeat a track until the whole playlist has played once. After that, the order is reshuffled. Off by default.");
