...
internal static partial class UiStrings
{
    ...
    public static string TipLibraryFavorites => Get(
        "お気に入り。名前だけ表示（フルパスは出さない）。複数選択可。プレイリストへドラッグで追加。外へドラッグ、または Ctrl+C／右クリックのコピーで、実ファイルをコピーできる。Enter でプレイリストをクリアして追加し即再生（見つけ次第、1曲ずつ。ツリーと同じ）。Shift+Enter とダブルクリックは追加のみ（再生しない。見つけ次第、1曲ずつ）。右クリックまたはメニューキーで、プレイリストをクリアして追加、プレイリストへ追加、お気に入りから削除、コピー、エクスプローラーで開く（登録を外してもファイルは消さない。Delete でも外せる）。F2 でここへフォーカス。Tab でプレイリストへ（Shift+Tab はツリー。フォーカス中は上端に明るいグラデーション）。テンキーの再生操作はここでも効く。",
        "Favorites. Names only (no full path). Multi-select. Drag onto the playlist to add. Drag outside the app, or Ctrl+C / right-click Copy, copies the real files. Enter clears the playlist, adds, and plays (one track at a time as found; same as the tree). Shift+Enter and double-click append only (do not play; one track at a time as found). Right-click or the menu key can clear the playlist and add, add to the playlist, remove from Favorites, copy, or open in Explorer (removing a favorite never deletes the file; Delete also unregisters it). F2 focuses here. Tab moves to the playlist (Shift+Tab to the tree; the active pane shows a brighter fade at the top). Numpad playback keys still work here.");
